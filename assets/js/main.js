...
const EXPERIENCE = [
  {
    ...
    bullets: [
      'Led a <strong>conversational AI test-case generation system</strong>: a QA engineer describes a test, the system returns an executable <strong>Airtest + Poco</strong> script grounded in a knowledge base of <strong>100K+ Unity prefab nodes</strong>, GM commands, protocol and config data mined from the game source.',
      'Closed the loop from generation → deterministic rewrites → static validation → <strong>false-pass audit</strong> → on-device run → knowledge feedback, raising the share of regression test points that yield a runnable, audit-clean script from roughly <strong>30% to 80%</strong> on the same samples. <a href="writing/knowing-what-the-button-is-called.html" class="inline-link">Write-up</a>.',
      'Built the front and back end of the internal <strong>automated testing platform</strong> (React + TypeScript / Flask + Socket.IO + MySQL) used by dozens of QA engineers, and shipped <strong>GM commands</strong> giving one-click entry into <strong>30+ game scenarios</strong> that used to need fragile UI navigation.',
    ],
  ...
  },
  {
    ...
    location: 'US, Remote',
    bullets: [
      'Built a <strong>multi-agent four-view image generation system</strong> (front / back / left / right from one reference): <strong>8 specialised agents</strong> in <strong>ReAct</strong> loops with MCP tool calling, where a separate evaluator judges each view and cross-view consistency, and only the failing views are regenerated. <strong>90%+</strong> of requests converge within <strong>2 iterations</strong>. <a href="writing/four-views-one-object.html" class="inline-link">Write-up</a>.',
      'Shipped <strong>Storyboard</strong> generation, prompt-to-workflow, and a video editing agent that turns conversation into executable node graphs with <strong>10+</strong> structured editing operations.',
    ],
  ...
  },
  {
    ...
    location: 'Shanghai, China',
    bullets: [
      'Designed and built an <strong>AI on-call assistant</strong> end to end (Python / React / SQLite): an LLM agent reads each incoming issue, classifies its intent and routes it to the right workflow. Adopted by <strong>1,000+ engineers</strong>; classification accuracy up <strong>80%</strong> over the manual process.',
      'Real-time architecture on <strong>WebSocket</strong> long connections with asynchronous LLM classification behind fault-tolerant queues, response caching and a persistence layer, so nothing is lost under load. A documentation-based interception step cut manual on-call requests by <strong>20%</strong>.',
      'Shipped a React dashboard for live request volume, ticket status and trends, and a reusable <strong>iOS Item Picker</strong> component with adaptive sheet and panel layouts.',
    ],
  ...
  },
  {
    ...
    bullets: [
      'Led an <strong>Android network diagnostics</strong> feature for Lazada in Southeast Asia, where weak and volatile networks were the top source of issue reports and no mature application-layer approach existed. Designed a detection strategy targeting a <strong>~30%</strong> reduction in network-related reports.',
      'Built the detection and diagnostic <strong>SDKs</strong> and UI in Kotlin and Java for three cases — blocked app network access, signal strength, and proxies — each paired with a fix the user can apply, so people in weak-network regions could self-diagnose.',
    ],
...
const PROJECTS = [
  {
    ...
    featured: true,
    bullets: [
      'An <strong>AI-native roguelike deck-builder</strong>: an LLM generates <strong>5 content types</strong> live during a run — cards, enemies, events, room rules, and characters — each through the same <strong>generate → validate → repair → retry</strong> loop.',
      ...
      'The budget is calibrated against real <em>Slay the Spire</em> cards, so "balanced" is measured, not guessed. Of <strong>2,658</strong> logged candidates, ~<strong>95%</strong> were served. Try the validator in the <a href="#playground" class="inline-link">Playground</a> or read <a href="writing/vibes-with-extra-steps.html" class="inline-link">how it was built</a>.',
    ],
  ...
  },
  {
    ...
    period: 'Jan – Mar 2026',
    bullets: [
      'Content moderation as a decision, not a label: a first pass extracts <strong>8+ structured risk signals</strong> (toxicity, uncertainty, context) and a <strong>ReAct agent</strong> picks a proportionate action — do nothing, downrank, add friction, or throttle.',
  ...
  },
  {
    ...
    period: 'Jun – Dec 2024',
    bullets: [
      'A <strong>U-Net</strong> with depthwise-separable and atrous convolutions for the Airbus ship dataset (50k+ satellite images), reaching <strong>Dice 0.63</strong> and <strong>precision 0.67</strong>. Ships are tiny relative to the image, so the work was in the losses (BCE, Dice, Focal) and the skip connections, not the backbone.',
      'Published as <strong>first author</strong> in <em>Applied and Computational Engineering</em>, CONF-SPML 2025.',
    ],
